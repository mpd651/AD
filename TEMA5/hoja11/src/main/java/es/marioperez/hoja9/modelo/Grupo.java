...
@NamedQueries({
	...
	@NamedQuery(name="Grupo.masCanciones", query="SELECT g FROM Grupo g where size(g.canciones)>?1"),
	@NamedQuery(name="Grupo.localidadPrimerDisco", query="SELECT g FROM Grupo g where g.localidad=?1 and g.annograb<=?2")
			})
